 ...
 */
 ...
 class theme_unitec_corporate_tbe_core_renderer extends theme_bootstrapbase_core_renderer {
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = unitec_corporate_tbe_performance_output($perf);
            }
        }
    ...
    protected function render_custom_menu(custom_menu $menu) {
    	...
    	*/
        ...
        if (isloggedin() && $hasdisplaymycourses) {
            ...
            if ($mycoursetitle == 'module') {
				$branchlabel = '<i class="icon-briefcase"></i>'.get_string('mymodules', 'theme_unitec_corporate_tbe');
				$branchtitle = get_string('mymodules', 'theme_unitec_corporate_tbe');
			} else if ($mycoursetitle == 'unit') {
				$branchlabel = '<i class="icon-briefcase"></i>'.get_string('myunits', 'theme_unitec_corporate_tbe');
				$branchtitle = get_string('myunits', 'theme_unitec_corporate_tbe');
			} else if ($mycoursetitle == 'class') {
				$branchlabel = '<i class="icon-briefcase"></i>'.get_string('myclasses', 'theme_unitec_corporate_tbe');
				$branchtitle = get_string('myclasses', 'theme_unitec_corporate_tbe');
			} else {
				$branchlabel = '<i class="icon-briefcase"></i>'.get_string('mycourses', 'theme_unitec_corporate_tbe');
				$branchtitle = get_string('mycourses', 'theme_unitec_corporate_tbe');
			}
            ...
            $branch = $menu->add($branchlabel, $branchurl, $branchtitle, $branchsort);
 			if ($courses = enrol_get_my_courses(NULL, 'fullname ASC')) {
 				...
 				$branch->add('<em>'.get_string('noenrolments', 'theme_unitec_corporate_tbe').'</em>',new moodle_url('/'),get_string('noenrolments', 'theme_unitec_corporate_tbe'));
 			}
    	...
    	*/
        ...
        if (isloggedin() && $hasdisplaymydashboard) {
            $branchlabel = '<i class="icon-dashboard"></i>'.get_string('mydashboard', 'theme_unitec_corporate_tbe');
            $branchurl   = new moodle_url('/my/index.php');
            $branchtitle = get_string('mydashboard', 'theme_unitec_corporate_tbe');
            $branchsort  = 10000;
